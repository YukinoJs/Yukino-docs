...
You can use additional built-in filters with `FilterUtil`:

```typescript
// Vibrato filter
await player.setFilters(FilterUtil.vibrato({ frequency: 5, depth: 0.7 }));

// Tremolo filter
await player.setFilters(FilterUtil.tremolo({ frequency: 4, depth: 0.5 }));

// Rotation filter (8D audio effect)
await player.setFilters(FilterUtil.rotation({ rotationHz: 0.2 }));

// Distortion filter
await player.setFilters(FilterUtil.distortion({ sinOffset: 0.5, sinScale: 0.5 }));

// Channel mix filter
await player.setFilters(FilterUtil.channelMix({ leftToLeft: 0.5, rightToRight: 0.5 }));

// Low pass filter
await player.setFilters(FilterUtil.lowPass({ smoothing: 10 }));
```
